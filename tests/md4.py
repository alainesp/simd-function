...
with Function(void)(state, block) as md4_block:
    ...
    # Round 2
    with Repeat(4):
        a += block[ 0]; t = d | c; tt = c & d; a += 0x5a827999; t &= b; t |= tt; a += t;     t = a <<  3; a >>= (32 -  3); a |= t
        d += block[ 4]; t = c | b; tt = b & c; d += 0x5a827999; t &= a; t |= tt; d += t;     t = d <<  5; d >>= (32 -  5); d |= t
        c += block[ 8]; t = b | a; tt = a & b; c += 0x5a827999; t &= d; t |= tt; c += t;     t = c <<  9; c >>= (32 -  9); c |= t
        b += block[12]; t = a | d; tt = d & a; b += 0x5a827999; t &= c; t |= tt; b += t;     t = b << 13; b >>= (32 - 13); b |= t
        block += 1
    block -= 4

    # Round 3
    for i in [0, 2, 1, 3]:
        a += block[i +  0]; t = d ^ c; a += 0x6ed9eba1; t ^= b; a += t;     t = a <<  3; a >>= (32 -  3); a |= t
        d += block[i +  8]; t = b ^ c; d += 0x6ed9eba1; t ^= a; d += t;     t = d <<  9; d >>= (32 -  9); d |= t
        c += block[i +  4]; t = a ^ b; c += 0x6ed9eba1; t ^= d; c += t;     t = c << 11; c >>= (32 - 11); c |= t
        b += block[i + 12]; t = a ^ d; b += 0x6ed9eba1; t ^= c; b += t;     t = b << 15; b >>= (32 - 15); b |= t
...
## <param name="state">The md4 state</param>
## <param name="block">The message to compress</param>
with Function(void)(state, block) as md4_block_avx512:
    # Load state
    a = state[0]
    b = state[1]
    c = state[2]
...
md4_block_avx512.targets = [Target.AVX512]
md4_block.parallelization_factor[Target.SSE2] = [3]
md4_block.parallelization_factor[Target.AVX ] = [3]
md4_block.parallelization_factor[Target.AVX2] = [3]
md4_block_avx512.parallelization_factor[Target.AVX512] = [4]
md4_block_avx512.name = 'md4_block'
# Build and run
generate_code()
