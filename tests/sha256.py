...
# Generator
def sha256_block_generator(state, W, consts, is_avx512):
    # Load state
    ...
    if is_avx512:
        ...
        H += (rotl(E, 26) ^ rotl(E, 21) ^ rotl(E, 7))  +  (G ^ (E & (F ^ G)))  +  consts[ 0] + W0 ;   D += H;   H += (rotl(A, 30) ^ rotl(A, 19) ^ rotl(A, 10))  +  ((A & B) | (C & (A | B)))
        G += (rotl(D, 26) ^ rotl(D, 21) ^ rotl(D, 7))  +  (F ^ (D & (E ^ F)))  +  consts[ 1] + W1 ;   C += G;   G += (rotl(H, 30) ^ rotl(H, 19) ^ rotl(H, 10))  +  ((H & A) | (B & (H | A)))
        F += (rotl(C, 26) ^ rotl(C, 21) ^ rotl(C, 7))  +  (E ^ (C & (D ^ E)))  +  consts[ 2] + W2 ;   B += F;   F += (rotl(G, 30) ^ rotl(G, 19) ^ rotl(G, 10))  +  ((G & H) | (A & (G | H)))
        E += (rotl(B, 26) ^ rotl(B, 21) ^ rotl(B, 7))  +  (D ^ (B & (C ^ D)))  +  consts[ 3] + W3 ;   A += E;   E += (rotl(F, 30) ^ rotl(F, 19) ^ rotl(F, 10))  +  ((F & G) | (H & (F | G)))
        D += (rotl(A, 26) ^ rotl(A, 21) ^ rotl(A, 7))  +  (C ^ (A & (B ^ C)))  +  consts[ 4] + W4 ;   H += D;   D += (rotl(E, 30) ^ rotl(E, 19) ^ rotl(E, 10))  +  ((E & F) | (G & (E | F)))
        C += (rotl(H, 26) ^ rotl(H, 21) ^ rotl(H, 7))  +  (B ^ (H & (A ^ B)))  +  consts[ 5] + W5 ;   G += C;   C += (rotl(D, 30) ^ rotl(D, 19) ^ rotl(D, 10))  +  ((D & E) | (F & (D | E)))
        B += (rotl(G, 26) ^ rotl(G, 21) ^ rotl(G, 7))  +  (A ^ (G & (H ^ A)))  +  consts[ 6] + W6 ;   F += B;   B += (rotl(C, 30) ^ rotl(C, 19) ^ rotl(C, 10))  +  ((C & D) | (E & (C | D)))
        A += (rotl(F, 26) ^ rotl(F, 21) ^ rotl(F, 7))  +  (H ^ (F & (G ^ H)))  +  consts[ 7] + W7 ;   E += A;   A += (rotl(B, 30) ^ rotl(B, 19) ^ rotl(B, 10))  +  ((B & C) | (D & (B | C)))
        H += (rotl(E, 26) ^ rotl(E, 21) ^ rotl(E, 7))  +  (G ^ (E & (F ^ G)))  +  consts[ 8] + W8 ;   D += H;   H += (rotl(A, 30) ^ rotl(A, 19) ^ rotl(A, 10))  +  ((A & B) | (C & (A | B)))
        G += (rotl(D, 26) ^ rotl(D, 21) ^ rotl(D, 7))  +  (F ^ (D & (E ^ F)))  +  consts[ 9] + W9 ;   C += G;   G += (rotl(H, 30) ^ rotl(H, 19) ^ rotl(H, 10))  +  ((H & A) | (B & (H | A)))
        F += (rotl(C, 26) ^ rotl(C, 21) ^ rotl(C, 7))  +  (E ^ (C & (D ^ E)))  +  consts[10] + W10;   B += F;   F += (rotl(G, 30) ^ rotl(G, 19) ^ rotl(G, 10))  +  ((G & H) | (A & (G | H)))
        E += (rotl(B, 26) ^ rotl(B, 21) ^ rotl(B, 7))  +  (D ^ (B & (C ^ D)))  +  consts[11] + W11;   A += E;   E += (rotl(F, 30) ^ rotl(F, 19) ^ rotl(F, 10))  +  ((F & G) | (H & (F | G)))
        D += (rotl(A, 26) ^ rotl(A, 21) ^ rotl(A, 7))  +  (C ^ (A & (B ^ C)))  +  consts[12] + W12;   H += D;   D += (rotl(E, 30) ^ rotl(E, 19) ^ rotl(E, 10))  +  ((E & F) | (G & (E | F)))
        C += (rotl(H, 26) ^ rotl(H, 21) ^ rotl(H, 7))  +  (B ^ (H & (A ^ B)))  +  consts[13] + W13;   G += C;   C += (rotl(D, 30) ^ rotl(D, 19) ^ rotl(D, 10))  +  ((D & E) | (F & (D | E)))
        B += (rotl(G, 26) ^ rotl(G, 21) ^ rotl(G, 7))  +  (A ^ (G & (H ^ A)))  +  consts[14] + W14;   F += B;   B += (rotl(C, 30) ^ rotl(C, 19) ^ rotl(C, 10))  +  ((C & D) | (E & (C | D)))
        A += (rotl(F, 26) ^ rotl(F, 21) ^ rotl(F, 7))  +  (H ^ (F & (G ^ H)))  +  consts[15] + W15;   E += A;   A += (rotl(B, 30) ^ rotl(B, 19) ^ rotl(B, 10))  +  ((B & C) | (D & (B | C)))
    consts += 16
    ...
    with Repeat(3):
        if is_avx512:
            ...
            W0  += (rotl(W14, 15) ^ rotl(W14, 13) ^ (W14 >> 10))  +  W9   +  (rotl(W1 , 25) ^ rotl(W1 , 14) ^ (W1  >> 3));   H += (rotl(E, 26) ^ rotl(E, 21) ^ rotl(E, 7))  +  (G ^ (E & (F ^ G)))  +  consts[ 0] + W0 ;   D += H;   H += (rotl(A, 30) ^ rotl(A, 19) ^ rotl(A, 10)) + ((A & B) | (C & (A | B)))
            W1  += (rotl(W15, 15) ^ rotl(W15, 13) ^ (W15 >> 10))  +  W10  +  (rotl(W2 , 25) ^ rotl(W2 , 14) ^ (W2  >> 3));   G += (rotl(D, 26) ^ rotl(D, 21) ^ rotl(D, 7))  +  (F ^ (D & (E ^ F)))  +  consts[ 1] + W1 ;   C += G;   G += (rotl(H, 30) ^ rotl(H, 19) ^ rotl(H, 10)) + ((H & A) | (B & (H | A)))
            W2  += (rotl(W0 , 15) ^ rotl(W0 , 13) ^ (W0  >> 10))  +  W11  +  (rotl(W3 , 25) ^ rotl(W3 , 14) ^ (W3  >> 3));   F += (rotl(C, 26) ^ rotl(C, 21) ^ rotl(C, 7))  +  (E ^ (C & (D ^ E)))  +  consts[ 2] + W2 ;   B += F;   F += (rotl(G, 30) ^ rotl(G, 19) ^ rotl(G, 10)) + ((G & H) | (A & (G | H)))
            W3  += (rotl(W1 , 15) ^ rotl(W1 , 13) ^ (W1  >> 10))  +  W12  +  (rotl(W4 , 25) ^ rotl(W4 , 14) ^ (W4  >> 3));   E += (rotl(B, 26) ^ rotl(B, 21) ^ rotl(B, 7))  +  (D ^ (B & (C ^ D)))  +  consts[ 3] + W3 ;   A += E;   E += (rotl(F, 30) ^ rotl(F, 19) ^ rotl(F, 10)) + ((F & G) | (H & (F | G)))
            W4  += (rotl(W2 , 15) ^ rotl(W2 , 13) ^ (W2  >> 10))  +  W13  +  (rotl(W5 , 25) ^ rotl(W5 , 14) ^ (W5  >> 3));   D += (rotl(A, 26) ^ rotl(A, 21) ^ rotl(A, 7))  +  (C ^ (A & (B ^ C)))  +  consts[ 4] + W4 ;   H += D;   D += (rotl(E, 30) ^ rotl(E, 19) ^ rotl(E, 10)) + ((E & F) | (G & (E | F)))
            W5  += (rotl(W3 , 15) ^ rotl(W3 , 13) ^ (W3  >> 10))  +  W14  +  (rotl(W6 , 25) ^ rotl(W6 , 14) ^ (W6  >> 3));   C += (rotl(H, 26) ^ rotl(H, 21) ^ rotl(H, 7))  +  (B ^ (H & (A ^ B)))  +  consts[ 5] + W5 ;   G += C;   C += (rotl(D, 30) ^ rotl(D, 19) ^ rotl(D, 10)) + ((D & E) | (F & (D | E)))
            W6  += (rotl(W4 , 15) ^ rotl(W4 , 13) ^ (W4  >> 10))  +  W15  +  (rotl(W7 , 25) ^ rotl(W7 , 14) ^ (W7  >> 3));   B += (rotl(G, 26) ^ rotl(G, 21) ^ rotl(G, 7))  +  (A ^ (G & (H ^ A)))  +  consts[ 6] + W6 ;   F += B;   B += (rotl(C, 30) ^ rotl(C, 19) ^ rotl(C, 10)) + ((C & D) | (E & (C | D)))
            W7  += (rotl(W5 , 15) ^ rotl(W5 , 13) ^ (W5  >> 10))  +  W0   +  (rotl(W8 , 25) ^ rotl(W8 , 14) ^ (W8  >> 3));   A += (rotl(F, 26) ^ rotl(F, 21) ^ rotl(F, 7))  +  (H ^ (F & (G ^ H)))  +  consts[ 7] + W7 ;   E += A;   A += (rotl(B, 30) ^ rotl(B, 19) ^ rotl(B, 10)) + ((B & C) | (D & (B | C)))
            W8  += (rotl(W6 , 15) ^ rotl(W6 , 13) ^ (W6  >> 10))  +  W1   +  (rotl(W9 , 25) ^ rotl(W9 , 14) ^ (W9  >> 3));   H += (rotl(E, 26) ^ rotl(E, 21) ^ rotl(E, 7))  +  (G ^ (E & (F ^ G)))  +  consts[ 8] + W8 ;   D += H;   H += (rotl(A, 30) ^ rotl(A, 19) ^ rotl(A, 10)) + ((A & B) | (C & (A | B)))
            W9  += (rotl(W7 , 15) ^ rotl(W7 , 13) ^ (W7  >> 10))  +  W2   +  (rotl(W10, 25) ^ rotl(W10, 14) ^ (W10 >> 3));   G += (rotl(D, 26) ^ rotl(D, 21) ^ rotl(D, 7))  +  (F ^ (D & (E ^ F)))  +  consts[ 9] + W9 ;   C += G;   G += (rotl(H, 30) ^ rotl(H, 19) ^ rotl(H, 10)) + ((H & A) | (B & (H | A)))
            W10 += (rotl(W8 , 15) ^ rotl(W8 , 13) ^ (W8  >> 10))  +  W3   +  (rotl(W11, 25) ^ rotl(W11, 14) ^ (W11 >> 3));   F += (rotl(C, 26) ^ rotl(C, 21) ^ rotl(C, 7))  +  (E ^ (C & (D ^ E)))  +  consts[10] + W10;   B += F;   F += (rotl(G, 30) ^ rotl(G, 19) ^ rotl(G, 10)) + ((G & H) | (A & (G | H)))
            W11 += (rotl(W9 , 15) ^ rotl(W9 , 13) ^ (W9  >> 10))  +  W4   +  (rotl(W12, 25) ^ rotl(W12, 14) ^ (W12 >> 3));   E += (rotl(B, 26) ^ rotl(B, 21) ^ rotl(B, 7))  +  (D ^ (B & (C ^ D)))  +  consts[11] + W11;   A += E;   E += (rotl(F, 30) ^ rotl(F, 19) ^ rotl(F, 10)) + ((F & G) | (H & (F | G)))
            W12 += (rotl(W10, 15) ^ rotl(W10, 13) ^ (W10 >> 10))  +  W5   +  (rotl(W13, 25) ^ rotl(W13, 14) ^ (W13 >> 3));   D += (rotl(A, 26) ^ rotl(A, 21) ^ rotl(A, 7))  +  (C ^ (A & (B ^ C)))  +  consts[12] + W12;   H += D;   D += (rotl(E, 30) ^ rotl(E, 19) ^ rotl(E, 10)) + ((E & F) | (G & (E | F)))
            W13 += (rotl(W11, 15) ^ rotl(W11, 13) ^ (W11 >> 10))  +  W6   +  (rotl(W14, 25) ^ rotl(W14, 14) ^ (W14 >> 3));   C += (rotl(H, 26) ^ rotl(H, 21) ^ rotl(H, 7))  +  (B ^ (H & (A ^ B)))  +  consts[13] + W13;   G += C;   C += (rotl(D, 30) ^ rotl(D, 19) ^ rotl(D, 10)) + ((D & E) | (F & (D | E)))
            W14 += (rotl(W12, 15) ^ rotl(W12, 13) ^ (W12 >> 10))  +  W7   +  (rotl(W15, 25) ^ rotl(W15, 14) ^ (W15 >> 3));   B += (rotl(G, 26) ^ rotl(G, 21) ^ rotl(G, 7))  +  (A ^ (G & (H ^ A)))  +  consts[14] + W14;   F += B;   B += (rotl(C, 30) ^ rotl(C, 19) ^ rotl(C, 10)) + ((C & D) | (E & (C | D)))
            W15 += (rotl(W13, 15) ^ rotl(W13, 13) ^ (W13 >> 10))  +  W8   +  (rotl(W0 , 25) ^ rotl(W0 , 14) ^ (W0  >> 3));   A += (rotl(F, 26) ^ rotl(F, 21) ^ rotl(F, 7))  +  (H ^ (F & (G ^ H)))  +  consts[15] + W15;   E += A;   A += (rotl(B, 30) ^ rotl(B, 19) ^ rotl(B, 10)) + ((B & C) | (D & (B | C)))
        consts += 16
...
with Function(void)(state, W) as sha256_block:
    sha256_block_generator(state, W, consts_simd, False)
...
## <param name="state">The SHA256 state</param>
## <param name="block">The message to compress</param>
with Function(void)(state, W) as sha256_block_avx2:
    sha256_block_generator(state, W, consts_scalar, False)
...
with Function(void)(state, W) as sha256_block_avx512:
    sha256_block_generator(state, W, consts_scalar, True)
    
# Define targets
sha256_block.targets = [Target.PLAIN_C, Target.SSE2, Target.AVX]
sha256_block_avx2.targets = [Target.AVX2]
sha256_block_avx512.targets = [Target.AVX512]
sha256_block_avx2.name = 'sha256_block'
